mask_no_key = input('enter encryption key... ');

% Read files and initialize
[x,fs] = audioread ('audioOriginal2.wav');
x = x(1:30*fs); x = x(:);
L = length(x);
N = 512;
win = blackman(N+1);
...
im = imread('./images/dc_water.png');
S = size(im);
row = 1;
column = 1;

count2 = 0;
mask = num2str(mask_no_key);
out = str2num(mask(1));
mask_no = zeros(1, out);
